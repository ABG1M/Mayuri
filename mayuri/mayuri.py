...
class Mayuri(Client):
    ...
    async def catch_up(self):
        ...
        while True:
            diff = await self.invoke(
                raw.functions.updates.GetDifference(pts=pts, date=date, qts=0)
            )
                ...
                self.dispatcher.updates_queue.put_nowait(
                    (
                        raw.types.UpdateNewMessage(
                            message=msg, pts=new_state.pts, pts_count=-1
                        ),
                        users,
                        chats,
                    )
    ...
    async def start_scheduler(self):
        ...
        if self.scheduler.state == RunState.stopped:
            ...
            await self.scheduler.add_schedule(
                self.adminlist_watcher, IntervalTrigger(seconds=21600)
            )
            # run every 5 minutes
            await self.scheduler.add_schedule(
                self.captcha_timeout_watcher, IntervalTrigger(seconds=60 * 5)
            )
            # run twice a week on monday and thursday at 6pm server time
            await self.scheduler.add_schedule(
                self.deleted_account_watcher,
                CronTrigger(day_of_week="mon,thu", hour=18),
            )
    ...
    async def adminlist_watcher(self):
        ...
        async for chat in chat_db.find():
            ...
            check = await db.find_one({"chat_id": chat_id})
            try:
                all_admin = self.get_chat_members(
            ...
            if check:
                for user_id in check["list"]:
                    if user_id not in admin_list:
                        await db.update_one(
                            {"chat_id": chat_id}, {"$pull": {"list": user_id}}
                        )
            second_loop = False
            for user_id in admin_list:
                if check or second_loop:
                    ...
                    if user_id not in check["list"]:
                        await db.update_one(
                            {"chat_id": chat_id}, {"$push": {"list": user_id}}
                        )
                else:
                    await db.update_one(
                        {"chat_id": chat_id}, {"$push": {"list": user_id}}, upsert=True
                    )
                    second_loop = True
    ...
    async def captcha_timeout_watcher(self):
        ...
        async for data in db.find():
            if data["timeout"] <= now:
                try:
                    if data["is_request"]:
                        await self.decline_chat_join_request(
    ...
    async def deleted_account_watcher(self):
        ...
        async for chat in db.find():
            ...
            async for member in chat_members:
                try:
                    if member.user.is_deleted and not await self.check_admin(
                        chat["chat_id"], member.user.id
                    ):
                        count = count + 1
            ...
            if count > 0:
                await self.send_message(
                    chat["chat_id"],
                    (await self.tl(chat["chat_id"], "zombies_cleaned_schedule")).format(
                        count, next_clean
                    ),
                )
