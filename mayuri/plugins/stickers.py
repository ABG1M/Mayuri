...
from pyrogram.raw.functions.messages import GetMessages as GetUserMessages
...
async def cmd_stickerid(c, m):
    ...
    if m.reply_to_message and m.reply_to_message.sticker:
        await m.reply_text(
            (await c.tl(chat_id, "your_stickerid")).format(
...
async def cmd_getsticker(c, m):
    ...
    if m.reply_to_message and m.reply_to_message.sticker:
        ...
        filename = "images/"
        filename += (
            "sticker.tgs" if animated else ("sticker.webm" if videos else "sticker.png")
        )
        ...
        if animated:
            ...
            await c.send_chat_action(
                m.chat.id, enums.ChatAction.UPLOAD_VIDEO, message_thread_id=thread_id
            )
            await m.reply_video(video=open(filename, "rb"))
            await m.reply_document(document=open(filename, "rb"), force_document=True)
        else:
            await c.send_chat_action(
                m.chat.id, enums.ChatAction.UPLOAD_PHOTO, message_thread_id=thread_id
            )
            await m.reply_photo(photo=open(filename, "rb"))
            await c.send_chat_action(
                m.chat.id, enums.ChatAction.UPLOAD_DOCUMENT, message_thread_id=thread_id
            )
            await m.reply_document(document=open(filename, "rb"), force_document=True)
        os.remove(filename)
    else:
...
async def cmd_kang(c, m):
    ...
    if reply and reply.media:
        ...
        while not packname_found:
            ...
            if not reply.sticker.file_name.endswith(
                ".tgs"
            ) and not reply.sticker.file_name.endswith(".webm"):
                resize = True
        else:
            return await prog_msg.edit_text(await c.tl(chat_id, "cannot_kang"))
        ...
        if filename:
            filename = await c.download_media(
                m.reply_to_message, "downloads/" + filename
            )
        else:
...
async def cmd_delsticker(c, m):
    ...
    if reply and reply.sticker:
        ...
        return await prog_msg.edit(
            (await c.tl(chat_id, "stickerdeleted")), reply_markup=markup
        )
