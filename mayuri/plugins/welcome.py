...
from mayuri.util.string import build_keyboard, parse_button
...
async def welcome(c, m):
    ...
    if not check:
        return await m.reply_text(await c.tl(chat_id, "welcome_not_set"))
    welc_settings = (await c.tl(chat_id, "welcome_settings")).format(
        check["enable"],
        check["clean_service"],
        check["thread_id"],
        check["is_captcha"],
        check["captcha_timeout"],
        check["verify_text"],
    )
    media = check["media"]
...
async def raw_welcome(c, u, _, __):
    if isinstance(u, raw.types.UpdateChannelParticipant):
        if hasattr(u, "new_participant") and isinstance(
            u.new_participant, raw.types.ChannelParticipant
        ):
            chat_id = int(f"-100{u.channel_id}")
            user_id = u.new_participant.user_id
            chat_raw = await c.invoke(
    ...
    for new_member in new_members:
                ...
                button.append(
                    [
                        InlineKeyboardButton(
                            verify_text,
                            url=f"https://t.me/{c.me.username}?start=verify_{verify_id}",
                        )
                    ]
                )
            else:
        ...
        if is_request:
            ...
            if chat.type == enums.ChatType.PRIVATE:
                if media:
                    ...
                    wc_msg = await c.send_message(
                        new_member.id, text=welcome_text, reply_markup=button
                    )
                return await captcha_db.insert_one(
                    {
                        "verify_id": verify_id,
                        "chat_id": chat_id,
            ...
            if media:
                if media_type == 0:
                    wc_msg = await c.send_photo(
                        chat_id, photo=media, caption=welcome_text, reply_markup=button
                    )
                elif media_type == 1:
                    wc_msg = await c.send_video(
                        chat_id, video=media, caption=welcome_text, reply_markup=button
                    )
                elif media_type == 2:
                    wc_msg = await c.send_animation(
                        chat_id,
                ...
                wc_msg = await c.send_message(
                    chat_id, text=welcome_text, reply_markup=button
                )
            return await captcha_db.insert_one(
                {
                    "verify_id": verify_id,
        ...
        if media:
                ...
                if chat.is_forum:
                    try:
                        wc_msg = await c.send_photo(
                            ...
                            photo=media,
                            caption=welcome_text,
                            message_thread_id=check["thread_id"],
                            reply_markup=button,
                        )
                    except Exception:
                        pass
            ...
            if chat.is_forum:
                ...
                wc_msg = await c.send_message(
                    chat_id=chat_id, text=welcome_text, reply_markup=button
                )
        if check["is_captcha"]:
            msg_id = wc_msg.id
            await captcha_db.insert_one(
                {
                    "verify_id": verify_id,
                    "chat_id": chat_id,
                    "user_id": new_member.id,
    ...
    if check and not check["is_captcha"]:
        ...
        if media:
            if media_type == 0:
                ...
                return await m.reply_photo(
                    photo=media, caption=welcome_text, reply_markup=button
                )
            if media_type == 1:
                if m.chat.is_forum:
                ...
                return await m.reply_video(
                    video=media, caption=welcome_text, reply_markup=button
                )
            if media_type == 0:
                if m.chat.is_forum:
                    return await c.send_animation(
                        chat_id,
                        animation=media,
                        caption=welcome_text,
                        message_thread_id=check["thread_id"],
                ...
                return await m.reply_animation(
                    animation=media, caption=welcome_text, reply_markup=button
                )
        if m.chat.is_forum:
...
@Mayuri.on_message(
    filters.group & filters.command("welcomecaptcha", PREFIX) & admin_only
)
