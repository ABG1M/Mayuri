...
async def addbl(c, m):
    ...
    await db.update_one(
        ...
        {"$set": {"mode": mode, "duration": duration, "reason": reason}},
        upsert=True,
    )
    ...
    if len(text) > 1:
        ...
        if check:
            ...
            return await m.reply_text(
                (await c.tl(chat_id, "blacklist_deleted")).format(trigger),
                disable_web_page_preview=True,
            )
        return await m.reply_text(
            (await c.tl(chat_id, "blacklist_not_found")).format(trigger),
            disable_web_page_preview=True,
        )
    ...
    if m.photo or m.sticker or (m.document and m.document.mime_type in mt):
        ...
        if c.config["blacklist"]["USE_OCR"]:
            ...
            try:
                ...
                text_from_img = await utils.run_sync(
                    pytesseract.image_to_string, im, config="--oem 3 --psm 12"
                )
        ...
        for data in data_list:
            if data["mode"] == mode:
                ...
                if not curr_duration_raw:
                    ...
                    duration_time = int(
                        curr_duration_raw[0 : len(curr_duration_raw) - 1]
                    )
                    if not curr_duration:
                        ...
                        and curr_unit == "s"
                        or duration_unit == "h"
                        and curr_unit in ["s", "m"]
                        or duration_unit == "d"
                        and curr_unit in ["s", "m", "h"]
                    ):
                        curr_duration = duration_time
                        curr_unit = duration_unit
                        duration_raw = curr_duration_raw
    ...
    if mode == 1:
        ...
        if duration_raw:
            await c.restrict_chat_member(
                chat_id, user_id, ChatPermissions(), datetime.fromtimestamp(duration)
            )
