...
@Mayuri.on_message(
    filters.command("alive", PREFIX) | filters.command("on", PREFIX) & sudo_only
)
async def alive(c, m):
...
async def create_captcha(c, m, db, verify_id, user_id):
    ...
    buttons.append(
        [InlineKeyboardButton("🔄", callback_data=f"_captcha_regen_{verify_id}")]
    )
    await db.update_one({"verify_id": verify_id}, {"$set": {"answer": answer}})
    return await m.reply_photo(
        photo=file_name,
        caption=(await c.tl(user_id, "select_all_emojis")),
...
async def start_msg(c, m):
            ...
            if data:
                ...
                if check and "rules" in check:
                    ...
                    button = [
                        [
                            InlineKeyboardButton(
                                text=(await c.tl(chat_id, "accept")),
                                callback_data=f"accept_rules_{verify_id}",
                            ),
                            InlineKeyboardButton(
                                text=(await c.tl(chat_id, "decline")),
                                callback_data=f"decline_rules_{verify_id}",
                            ),
                        ]
                    ]
                ...
                button = [
                    [
                        InlineKeyboardButton(
                            ...
                            callback_data=f"captcha_tunstile_{verify_id}",
                        )
                    ],
                    [
                        InlineKeyboardButton(
                            text="Emoji Captcha",
                            callback_data=f"captcha_emoji_{verify_id}",
                        )
                    ],
                ]
                return await m.reply_text(
                    text=(await c.tl(chat_id, "select_captcha_type")),
    ...
    text += "Powered by [Pyrofork v{}](https://pyrofork.mayuri.my.id)".format(
        __version__
    )
...
async def _captcha_tunstile(c, q):
    ...
    button = [
        [
            KeyboardButton(
                text=(await c.tl(chat_id, "verify")),
                web_app=WebAppInfo(
                    url=f"{c.config['captcha']['TURNSTILE_URL']}/?verify_id={verify_id}"
                ),
            )
        ]
    ]
    await m.reply_text(
        text=(await c.tl(chat_id, "press_verify")),
...
async def _emoji_captcha(c, q):
    ...
    msg = await m.edit_text(
        text=(await c.tl(chat_id, "generate_captcha")), reply_markup=None
    )
...
async def accept_rules(c, q):
    ...
    if check:
        button = [
            ...
            [
                InlineKeyboardButton(
                    text="Emoji Captcha", callback_data=f"captcha_emoji_{verify_id}"
                )
            ],
        ]
        await m.reply_text(
...
async def help_parser(c, chat_id, text, keyboard=None):
    if not keyboard:
        keyboard = InlineKeyboardMarkup(
            await paginate_plugins(c, 0, HELP_COMMANDS, "help", chat_id)
        )
    await c.send_message(
        chat_id, text, parse_mode=enums.ParseMode.MARKDOWN, reply_markup=keyboard
    )
    ...
    if m.chat.type != enums.ChatType.PRIVATE:
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text=await c.tl(chat_id, "helps"),
                        url=f"t.me/{(await c.get_me()).username}?start=help",
                    )
                ]
            ]
        )
        ...
        return
    await help_parser(
        c, m.chat.id, (await c.tl(chat_id, "HELP_STRINGS")).format(", ".join(PREFIX))
    )
...
async def help_button(c, q):
    ...
    if mod_match:
        ...
        text = (await c.tl(chat_id, "this_plugin_help")).format(
            await c.tl(chat_id, HELP_COMMANDS[plugin].__PLUGIN__)
        ...
        await q.message.edit(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text=await c.tl(chat_id, "back"), callback_data="help_back"
                        )
                    ]
                ]
            ),
        ...
        await q.message.edit(
            text=(await c.tl(chat_id, "HELP_STRINGS")).format(", ".join(PREFIX)),
            reply_markup=InlineKeyboardMarkup(
                await paginate_plugins(c, 0, HELP_COMMANDS, "help", chat_id)
            ),
        )
...
async def set_language(c, m):
    ...
    if m.chat.type != enums.ChatType.PRIVATE and not await c.check_admin(
    ...
    for lang in list_all_lang():
        ...
        i += 1
    await m.reply_text(
        text=(await c.tl(chat_id, "select_lang")),
        reply_markup=InlineKeyboardMarkup(buttons),
    )
...
async def set_lang_respond(c, q):
    ...
    m = q.message
    if m.chat.type != enums.ChatType.PRIVATE and not await c.check_admin(
        m.chat.id, q.from_user.id
    ):
        return await c.answer_callback_query(
            callback_query_id=q.id, text="You're not admin!", show_alert=True
        )
    lang = q.data[8:]
