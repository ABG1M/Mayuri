...
async def chat_watcher(c, m):
    ...
    await db.update_one(
        {"chat_id": chat_id},
        {"$set": {"chat_username": m.chat.username, "chat_name": m.chat.title}},
        upsert=True,
    )
...
async def cas_watcher(c, m):
    ...
    try:
        ...
        pass
    if not await c.check_admin(chat_id, user_id) and not await c.check_approved(
        chat_id, user_id
    ):
        await c.ban_chat_member(int(chat_id), user_id)
    await db.update_one(
        {"user_id": user_id}, {"$set": {"reason": reason, "until": 0}}, upsert=True
    )
    async for chat in chat_db.find():
        if chat["chat_id"] != m.chat.id:
            try:
                if not await c.check_admin(
    ...
    log = (await c.tl(chat_id, "cas_log")).format(
        chat["chat_username"], mention, user_id, reason
    )
...
async def gban_task(c, m):
    ...
    if m.reply_to_message:
        ...
        if len(text) > 1:
            ...
            if len(extracted) > 1:
                ...
                elif len(extracted1) > 1 and re.match(
                    r"([0-9]{1,})([dhms])$", extracted1[0].lower()
                ):
    ...
    await db.update_one(
        {"user_id": user.id}, {"$set": {"reason": reason, "until": until}}, upsert=True
    )
    async for chat in chat_db.find():
        if await c.check_admin(chat["chat_id"], user.id) or await c.check_approved(
        ...
        try:
            if duration:
                await c.ban_chat_member(
                    chat["chat_id"], user.id, datetime.fromtimestamp(until)
                )
            else:
...
async def gban_watcher(c, m):
    ...
    mention = m.from_user.mention
    now = time.time()
    if await c.check_admin(chat_id, user_id) or await c.check_approved(
        chat_id, user_id
    ):
        return
    ...
    async for chat in chat_db.find():
        ...
        try:
            if duration:
                ...
                await c.restrict_chat_member(
                    chat["chat_id"], user.id, ChatPermissions()
                )
        except RPCError:
            print(RPCError)
    ...
    if until != 0:
        if until > now:
            if until - now < 40:
                until = now + 40
            await c.restrict_chat_member(
                chat_id, user_id, ChatPermissions(), datetime.fromtimestamp(until)
            )
        else:
            return await db.delete_one({"user_id": user_id})
    else:
