...
async def getCAPTCHAJPG(user_id):
    ...
    while True:
        ...
        position = (
            (base_img.width - positions[i][0]),
            (base_img.height - positions[i][1]),
        )
        base_img.paste(
            Image.open(rf"mayuri/images/emojis/{EmojiBank[EmojiRandomSelect]}.png")
            .resize((60, 60))
            .rotate(rotations[i]),
            position,
            Image.open(rf"mayuri/images/emojis/{EmojiBank[EmojiRandomSelect]}.png")
...
async def gen_button(verify_id, answer):
    ...
    for emo in all_emoji:
        buttons[j].append(
            InlineKeyboardButton(emo, callback_data=f"_captcha_{verify_id}_{emo}")
        )
        if i % 4 == 0:
            j = j + 1
        i = i + 1
...
async def reload_captcha(c, m, db, chat_id, verify_id, user_id, msg_id):
    ...
    await m.edit_media(
        media=InputMediaPhoto(
            media=file_name, caption=(await c.tl(user_id, "select_all_emojis"))
        ),
...
async def check_respond(c, q):
    ...
    if regen:
        ...
        if check:
            ...
            return await c.loop.create_task(
                reload_captcha(c, m, db, chat_id, verify_id, user_id, msg_id)
            )
    ...
    if check and check["user_id"] == user_id:
        ...
        if wrong >= 3:
            return await c.loop.create_task(
                reload_captcha(m, db, check["chat_id"], verify_id, user_id, msg_id)
            )
        if right >= 6:
            ...
            try:
                ...
                if msg:
                    ...
                    welcome_set = await welcome_db.find_one(
                        {"chat_id": check["chat_id"]}
                    )
                    ...
                    if welcome_set["media"]:
                        return await msg.edit_caption(
                            caption=welcome_text, reply_markup=buttons
                        )
