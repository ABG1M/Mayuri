    ...
    if len(text) > 1:
        ...
        if cmd not in DISABLEABLE:
            return await m.reply_text(
                (await c.tl(chat_id, "cmd_not_found")).format(cmd),
        ...
        if check and "disabled_list" in check and cmd in check["disabled_list"]:
            return await m.reply_text(
                (await c.tl(chat_id, "cmd_already_disabled")).format(cmd),
                disable_web_page_preview=True,
            )
        await db.update_one(
            {"chat_id": chat_id}, {"$push": {"disabled_list": cmd}}, upsert=True
        )
        return await m.reply_text(
            (await c.tl(chat_id, "cmd_disabled")).format(cmd),
            disable_web_page_preview=True,
        )
    return m.reply_text(
        await c.tl(chat_id, "what_cmd_to_disable"), disable_web_page_preview=True
    )
...
async def rm_from_disabled(c, m):
    ...
    if len(text) > 1:
        ...
        if not check:
            return await m.reply_text(
                (await c.tl(chat_id, "cmd_not_disabled")).format(cmd),
                disable_web_page_preview=True,
            )
        ...
        if cmd in check["disabled_list"]:
            ...
            return await m.reply_text(
                (await c.tl(chat_id, "cmd_enabled")).format(cmd),
                disable_web_page_preview=True,
            )
        return await m.reply_text(
            (await c.tl(chat_id, "cmd_not_disabled")).format(cmd),
    ...
    return m.reply_text(
        await c.tl(chat_id, "what_cmd_to_enable"), disable_web_page_preview=True
    )
