...
from mayuri.util.string import build_keyboard, parse_button, split_quotes
...
async def addfilter(c, m):
    ...
    await m.reply_text(
        (await c.tl(chat_id, "filter_added")).format(name, m.chat.title),
...
async def rm_filter(c, m):
    ...
    if len(text) > 1:
        ...
        if check:
            ...
            await m.reply_text(
                (await c.tl(chat_id, "filter_removed")).format(name, m.chat.title),
                disable_web_page_preview=True,
            )
        else:
            await m.reply_text(
                (await c.tl(chat_id, "filter_not_found")).format(name),
                disable_web_page_preview=True,
            )
    else:
...
async def cmd_filters(c, m):
    ...
    if list_name:
        ...
        await m.reply_text(
            (await c.tl(chat_id, "no_filter_found")).format(m.chat.title)
        )
...
async def filter_watcher(c, m):
    ...
    async for filt in check:
        if filt["name"] in text:
            ...
            if filt["type"] == 1:
                await m.reply_text(text, reply_markup=button)
            elif filt["type"] == 3:
                await m.reply_sticker(filt["media"])
            elif filt["type"] == 4:
                if filt["media_type"] == 1:
                    await m.reply_audio(
                        audio=filt["media"], caption=text, reply_markup=button
                    )
                elif filt["media_type"] == 2:
                    await m.reply_document(
                        document=filt["media"], caption=text, reply_markup=button
                    )
                elif filt["media_type"] == 3:
                    await m.reply_photo(
                        photo=filt["media"], caption=text, reply_markup=button
                    )
                elif filt["media_type"] == 4:
                    await m.reply_video(
                        video=filt["media"], caption=text, reply_markup=button
                    )
            break
